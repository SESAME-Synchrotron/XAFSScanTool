...
class ConfigGUI:
    ...
    def __init__(self,paths):
        ...
        self.guiObj.tuning.nextId = self.tuning
        ...
        self.guiObj.RCEnabled.stateChanged.connect(self.RCFields)
    ...
    def RCFields(self, state): 
        if state == 2:  
            self.guiObj.RCConduct.setEnabled(True)
        else:  
            self.guiObj.RCConduct.setEnabled(False)
    ...
    def checkScanType(self):
        if self.guiObj.stepEngScan.isChecked():
            ...
            return self.WizardPages.cfgFile.value
        elif self.guiObj.contScanPosition.isChecked():
            self.scanTypeValue = 'contScanPosition'
            self.cfg['scanType'] = 'contScanPosition'
            return self.WizardPages.cfgFile.value
        elif self.guiObj.contScanTime.isChecked():
            self.scanTypeValue = 'contScanTime'
            self.cfg['scanType'] = 'contScanTime'
            return self.WizardPages.cfgFile.value
        else:
            return self.WizardPages.scanType.value
    ...
    def cfgfile(self):
        if self.guiObj.Create.isChecked():
            ...
            if self.scanTypeValue in {'stepEngScan', 'contScanPosition', 'contScanTime'}: 
    ...
    def loadcfg(self):
        ...
        if not path == "":
            self.cfg["loadedConfig"] = "Yes"
            # contScan below was added to be able to load old version config files
            if self.scanTypeValue in {'stepEngScan', 'contScanPosition', 'contScan', 'contScanTime'}:
                return self.WizardPages.stepContEngScanParameters.value
            else:
                return self.WizardPages.stepMapScanParameters.value
        else:
    ...
    def tuning(self):
        if self.guiObj.ICsSensitivity.checkState() == QtCore.Qt.Checked:
            self.cfg['ICsSensitivityTuning'] = 'Yes'
        else:
            self.cfg['ICsSensitivityTuning'] = 'No'
        
        if self.guiObj.RCEnabled.checkState() == QtCore.Qt.Checked:
            self.cfg['RockingCurveTuning'] = 'Yes'
            # RCConduct {0, 1, 2} 0: at start of experiment, 1: start of each sample, 2: at start of each scan 
            RCConduct = self.guiObj.RCConduct.currentText()
            self.cfg['RCConduct'] = RCConduct

        else:
            self.cfg['RockingCurveTuning'] = 'No'
    ...
    def editIntervals(self):
        ...
        if Common.regexvalidation("NIntervals", Nintrv):
            ...
            self.IntervalsGUI.setStepEV(self.scanTypeValue)
    ...
    def browseCfgFile(self):
        ...
        elif self.scanTypeValue in {'contScanPosition', 'contScanTime'}:
            try: 
                self.browseStepContEngScanCfgFile()
            except: 
    ...
    def browseStepContEngScanCfgFile(self):
        try:
            ...
            try:
                ...
                if self.scanTypeValue == 'stepEngScan':
                    self.cfg['scanType'] = 'stepEngScan'
                    # self.scanTypeValue = 'stepEngScan' #iii
                elif self.scanTypeValue == 'contScanPosition':
                    self.cfg['scanType'] = 'contScanPosition'
                    # self.scanTypeValue = 'contScanPosition' #iii
                elif self.scanTypeValue == 'contScanTime':
                    self.cfg['scanType'] = 'contScanTime'
            except:
                CLIMessage("Could not locate the config file", "W")
                return self.WizardPages.stepContEngScanParameters.value
    ...
    def checkStepContEngScanConfig(self):
        os.system('clear')
        if self.scanTypeValue in {"contScanPosition", "contScanTime"}:
        ...
        try:
            ...
            if self.scanTypeValue == "contScanPosition":
                ...
                for interval in range(self.IntervalsGUI.interval_UI.tableWidget.rowCount()):
                    start = self.IntervalsGUI.interval_UI.tableWidget.item(interval, 0).text()
                    evPerSec = self.IntervalsGUI.interval_UI.tableWidget.item(interval, 2).text()
                    intTime = self.IntervalsGUI.interval_UI.tableWidget.item(interval, 3).text()
                    speed = DCM.energySpeed (float(start),float(evPerSec),float(intTime))
                    CLIMessage("Interval number {} | Calculated speed: {}".format(interval +1, speed), 'I')
                    if float(minThetaSpeed) > float (speed):
                            CLIMessage("Intervals | please review continuous scan parameters of the interval number ({})".format(interval+1), 'E')
                            CLIMessage("Intervals | The desired mono speed is lower than the minimum limit.", 'E')
                            CLIMessage("Intervals | Start, ev/s or IC integration time can be changed to calculating acceptable speed", 'W')
                            return self.WizardPages.stepContEngScanParameters.value
                    elif float(maxThetaSpeed) < float (speed):
                            CLIMessage("Intervals | please review continuous scan parameters of the interval number ({})".format(interval+1), 'E')
                            CLIMessage("Intervals | The desired mono speed is greater than the max limit.", 'E')
                            CLIMessage("Intervals | Start, ev/s or IC integration time can be changed to calculating acceptable speed", 'W')
                            return self.WizardPages.stepContEngScanParameters.value

            return self.WizardPages.tuning.value
        except Exception as e:
            print ("Check config", e)
            return self.WizardPages.stepContEngScanParameters.value			
...
class IntervalGUI:
    ...
    def setStepEV(self, scanType): 
        if scanType == 'contScanTime':
            self.interval_UI.tableWidget.setHorizontalHeaderItem(2, QtWidgets.QTableWidgetItem("eV/s"))
        else:
            self.interval_UI.tableWidget.setHorizontalHeaderItem(2, QtWidgets.QTableWidgetItem("step"))
