...
1. re-enable file transfer
2. change fixed directory to experiment directory when finding xdi files
3. Add Offset PV
"""
...
class XAFS_XRF:
	...
	def pauseTrigger(self):
		...
		ICI0LowerLimit = self.scanLimits["ICI0LowerLimit"]
		KetekROI0LowerLimit = self.scanLimits["KetekROI0LowerLimit"]
		FicusROI0LowerLimit = self.scanLimits["FicusROI0LowerLimit"]
		...
		while True:
			...
			if shutter1Status == 3: # shutter is open 1, closed 0
				...
				if shutter1LogFlag == 1:
					log.warning("Shutter1 status is returned to allowed limits, now it is: open")
					shutter1LogFlag = 0
			else:
				shutter1Ok = False
			...
			if shutter2Status == 3: # shutter is open 1, closed 0
				...
				if shutter2LogFlag == 1:
					log.warning("Shutter2 status is returned to allowed limits, now it is: open")
					shutter2LogFlag =0
			else:
				shutter2Ok = False
			...
			if StopperStatus == 3: # stopper is open 1, closed 0
				stopperOk = True
				if stopperLogFlag == 1:
					log.warning("stopper status is returned to allowed limits, now it is: open")
					stopperLogFlag = 0
			else:
				stopperOk = False
				if stopperLogFlag == 0:
			...
			if currentOk and shutter1Ok and shutter2Ok and stopperOk == True:
				...
				if ICI0Readout >= ICI0LowerLimit:
					ICI0Ok = True
					if ICI0LogFlag == 1:
						log.warning("ICI0 value is returned to allowed limits, it is now {}"
							.format(ICI0Readout))
						ICI0LogFlag = 0
				else:
					ICI0Ok = False
				...
				if "KETEK" in self.detChosen:
					if KetekROI0Readout >= KetekROI0LowerLimit:
						...
						if KetekROI0LogFlag == 1:
							log.warning("ketek_ROI_0 value is returned to allowed limits, it is now {}"
				...
				if "FICUS" in self.detChosen:
					if FicusROI0Readout[0] >= FicusROI0LowerLimit:
						...
						if FicusROI0LogFlag == 1:
							log.warning("FICUS_ROI_0 value is returned to allowed limits, it is now {}"
	...
	def initDetectors(self):
		...
		oneIC = False
		for det in detlist:
			if det in ("IC1", "IC2", "IC3") and not oneIC:
				oneIC = True
